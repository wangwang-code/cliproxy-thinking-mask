...
func newUtlsRoundTripper(proxyURL string, handshakeTimeout ...time.Duration) *utlsRoundTripper {
	...
	if len(handshakeTimeout) > 0 && handshakeTimeout[0] > 0 {
		rt.handshakeTimeout = handshakeTimeout[0]
	}
	return rt
}
...
func (t *utlsRoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if t.handshakeTimeout > 0 {
		connectCtx, cancel := context.WithTimeout(req.Context(), t.handshakeTimeout)
		h2Conn, err = t.createConnection(connectCtx, hostname, addr)
		cancel()
	} else {
...
func NewUtlsHTTPClient(ctx context.Context, cfg *config.Config, auth *cliproxyauth.Auth, timeout time.Duration) *http.Client {
	...
	var chromeRT http.RoundTripper = newUtlsRoundTripper(proxyURL, CodexFirstEventTimeout(cfg))
