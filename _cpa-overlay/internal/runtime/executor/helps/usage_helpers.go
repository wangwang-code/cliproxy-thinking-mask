...
// StreamLimitUsagePublisherContextKey is the gin.Context key used to publish a
// failed usage record when a downstream stream budget aborts the request.
const StreamLimitUsagePublisherContextKey = "cpaStreamLimitUsagePublisher"

// BindStreamLimitPublisher stores a callback on the gin context that publishes a
// failed usage record for a locally aborted stream. Call it once after the
// reporter is created and before streaming begins.
func (r *UsageReporter) BindStreamLimitPublisher(ctx context.Context) {
	if r == nil || ctx == nil {
		return
	}
	...
	ginCtx.Set(StreamLimitUsagePublisherContextKey, func(err error) {
		if err != nil {
			r.PublishFailure(ctx, err)
		}
	})
}
