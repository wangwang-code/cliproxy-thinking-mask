...
type CodexConfig struct {
	...
	// ConnectionRetry is how many times a codex upstream request is retried in place
	// when the connection fails at the transport level before any HTTP response is
	// received (EOF, connection refused/reset, dial/timeout). This absorbs transient
	// TLS/transport failures without rotating credentials or surfacing the error to
	// the client. <= 0 disables the in-place retry. Default is 0 (disabled).
	ConnectionRetry int `yaml:"connection-retry,omitempty" json:"connection-retry,omitempty"`
	// ConnectionRetryInterval is the delay between in-place connection retries
	// (time.ParseDuration, e.g. "500ms", "1s"). Empty/invalid falls back to 300ms.
	ConnectionRetryInterval string `yaml:"connection-retry-interval,omitempty" json:"connection-retry-interval,omitempty"`
	// RefreshRetries is how many attempts the codex token refresh makes per refresh
	// call when the upstream rejects with a transient error. <= 0 disables retries
	// (single attempt). Default is 3 (matching the historical fixed value).
	RefreshRetries int `yaml:"refresh-retries,omitempty" json:"refresh-retries,omitempty"`
	// RefreshRetryInterval is the base delay before the first refresh retry
	// (time.ParseDuration). Subsequent attempts scale linearly (attempt * interval)
	// to keep backoff bounded. Empty/invalid falls back to 1s.
	RefreshRetryInterval string `yaml:"refresh-retry-interval,omitempty" json:"refresh-retry-interval,omitempty"`
