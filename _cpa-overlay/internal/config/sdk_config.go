...
type StreamingConfig struct {
	...
	// FakeThinkingText is the reasoning text sent to a streaming chat.completions
	// client immediately after the SSE stream commits, while the upstream first
	// token is still pending. It only takes effect when KeepAliveSeconds > 0.
