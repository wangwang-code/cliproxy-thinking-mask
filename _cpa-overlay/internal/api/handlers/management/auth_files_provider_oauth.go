...
func (h *Handler) RequestCodexToken(c *gin.Context) {
	...
	// Initialize Codex auth service. When the caller provides a per-login proxy
	// (page top proxy input), it takes precedence over the global proxy-url for
	// every CPA outbound request of this login flow (authorization URL build,
	// code->token exchange). This avoids leaking the CPA host's direct egress and
	// lets each login use the proxy shown on its own page instead of editing the
	// global proxy-url.
	proxyURL := strings.TrimSpace(c.Query("proxy-url"))
	if proxyURL == "" {
		proxyURL = strings.TrimSpace(c.GetHeader("X-Proxy-URL"))
	}
	var openaiAuth codexOAuthService
	if proxyURL != "" {
