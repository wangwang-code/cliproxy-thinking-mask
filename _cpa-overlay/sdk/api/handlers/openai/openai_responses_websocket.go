...
type responsesWebsocketWriter struct {
	...
	writeMu sync.Mutex
	closing atomic.Bool
	rewrite func(*interfaces.ErrorMessage) *interfaces.ErrorMessage
}

func newResponsesWebsocketWriter(conn *websocket.Conn, rewrite ...func(*interfaces.ErrorMessage) *interfaces.ErrorMessage) *responsesWebsocketWriter {
	w := &responsesWebsocketWriter{conn: conn}
	if len(rewrite) > 0 && rewrite[0] != nil {
		w.rewrite = rewrite[0]
	}
	return w
}
...
func (h *OpenAIResponsesAPIHandler) ResponsesWebsocket(c *gin.Context) {
	...
	writer := newResponsesWebsocketWriter(conn, h.RewriteExecutionErrorMessage)
