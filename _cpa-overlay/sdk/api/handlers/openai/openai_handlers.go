...
// defaultFakeThinkingText is the reasoning text used when
// streaming.fake-thinking-text is unset.
const defaultFakeThinkingText = "Let me think through this step by step before giving the final answer."

// earlyThinkingDelta is the delta of the synthetic first chat.completion.chunk
// emitted before the upstream first token arrives. It carries only
// reasoning_content (thinking) so clients render the thinking phase; content is
// intentionally omitted.
type earlyThinkingDelta struct {
	Role             string `json:"role,omitempty"`
	ReasoningContent string `json:"reasoning_content,omitempty"`
}

type earlyThinkingChoice struct {
	Index        int                `json:"index"`
	Delta        earlyThinkingDelta `json:"delta"`
	FinishReason json.RawMessage    `json:"finish_reason"`
}

type earlyThinkingChunk struct {
	ID      string                `json:"id"`
	Object  string                `json:"object"`
	Created int64                 `json:"created"`
	Model   string                `json:"model"`
	Choices []earlyThinkingChoice `json:"choices"`
}
...
func buildEarlyThinkingChunk(model, thinkingText string) []byte {
	...
	chunk := earlyThinkingChunk{
		...
		Choices: []earlyThinkingChoice{
			{
				Index:        0,
				Delta:        earlyThinkingDelta{Role: "assistant", ReasoningContent: text},
				FinishReason: json.RawMessage("null"),
			},
		},
	}
