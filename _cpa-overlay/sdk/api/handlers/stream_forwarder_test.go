...
func TestForwardStreamStallTimeout(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodGet, "/", nil)
	...
	if written == "" || canceled == "" || written != canceled {
		t.Fatalf("written=%q canceled=%q, want same stall error", written, canceled)
	}
	if !strings.Contains(written, "stalled") {
		t.Fatalf("error = %q, want stall message", written)
	}
}
